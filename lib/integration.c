...
static int
apply_qag(T *q, real a, real b, real (*f)(real, void*), void *p, real *result)
{
	gsl_function F;
	Param param;
	int status;
	...
	F.function = G;
	F.params = &param;
	status = gsl_integration_qag(&F, a, b, EPSABS, EPSREL, LIMIT, q->key, q->w, &dresult, &abserr);
...
static int
apply_qng(__UNUSED T *q, real a, real b, real (*f)(real, void*), void *p, real *result)
{
	gsl_function F;
	Param param;
	int status;
	...
	F.function = G;
	F.params = &param;
	status = gsl_integration_qng(&F, a, b, EPSABS, EPSREL,  &dresult, &abserr, &neval);
	if (status != GSL_SUCCESS)
		ERR(CO_NUM, "staus: %s", gsl_strerror(status));
